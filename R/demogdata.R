...
demogdata <- function(data, pop, ages, years, type, label, name, lambda) {
  ...
  if (type == "population") {
    obj <- list(
      year = years, age = ages, pop = list(as.matrix(pop)), type = type,
      label = label, lambda = lambda
    )
    dimnames(obj$pop[[1]]) <- list(ages, years)
  } else {
    obj <- list(
      year = years, age = ages, rate = list(as.matrix(data)), pop = list(as.matrix(pop)), type = type,
      label = label, lambda = lambda
    )
...
#' @keywords hplot
#' @export
plot.demogdata <- function(
    x, series = ifelse(!is.null(x$rate), names(x$rate)[1], names(x$pop)[1]),
...
#' @export
median.demogdata <- function(
    x, na.rm = FALSE, series = names(x$rate)[1],
