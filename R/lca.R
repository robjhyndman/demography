...
lca <- function(
  ...
  if (chooseperiod) {
    if (breakmethod == "bai") {
      ...
      out <- lca(data, series, year[(bestbreak + 1):m],
        ages = ages, max.age = max.age,
  ...
  residuals <- fts(ages, t(res),
    frequency = 1 / deltat, start = years[1], xname = "Age",
    yname = paste("Residuals", data$type, "rate")
  )
  fitted <- fts(ages, t(fit),
    frequency = 1 / deltat, start = years[1], xname = "Age",
...
bms <- function(
  ...
  breakmethod <- match.arg(breakmethod)
  out <- lca(data,
    series = series, years = years, ages = ages, max.age = max.age, adjust = "dxt",
...
forecast.lca <- function(object, h = 50, se = c("innovdrift", "innovonly"), jumpchoice = c("fit", "actual"), level = 80, ...) {
  ...
  output <- list(
    label = object$label, age = object$age, year = object$year[nyears] + x * deltat,
    ...
    kt.f = structure(list(
      mean = kt.f[, 1], lower = kt.f[, 2], upper = kt.f[, 3], level = level, x = object$kt,
